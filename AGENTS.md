...
- Tooling configs live at the repo root (`.oxlintrc.json`, `.oxfmtrc.json`, `vitest.config.mts`, `tsconfig*.json`); build artifacts output to `.next/` or `out/` (ignored).
...
- `pnpm check` runs lint + format check; `pnpm fix` writes formatting and then runs Oxlint fixes; prefer before commits.
...
- Let Oxfmt handle formatting and import sorting; avoid manual tweaks that fight the formatter.
